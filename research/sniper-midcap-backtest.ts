...
// ── Pure math indicators (avoids DB/module resolution issues in standalone tsx) ──
...
function buildSnap(candles: Candle[]) {
  ...
  return {
    price: closes[n - 1],
    atr: atrVal,
    adx: 25, // dummy, detectSniper calculates its own ADX/sigma internally
...
async function getMidCapUniverse(): Promise<string[]> {
  ...
  // Filter: USDT pairs, $10M < vol < $200M (mid-caps), exclude mega-caps
  ...
  const midCaps = tickers
    ...
    .slice(0, 20) // Top 20 mid-caps

  console.log(`Selected ${midCaps.length} mid-cap coins:`)
  console.log(midCaps.map(t => `${t.symbol} ($${(t.amount24/1e6).toFixed(0)}M)`).join(", ") + "\n")
...
function runBacktest(candles: Candle[], sym: string): Trade[] {
  ...
  const tpMult = SNIPER_PARAMS.tpSlRatio ?? 4.0
  let lastSignalBar = -999

  // Start after 200 bars of warmup
  for (let i = 200; i < candles.length - 288; i++) {
    // Cooldown: max 1 signal per 12 bars (1 hour) per coin
    ...
    const tp = sig.direction === "long" ? entry + stopDist * tpMult : entry - stopDist * tpMult

    // Resolve trade
    for (let j = i + 1; j < Math.min(i + 288, candles.length); j++) {
      if (sig.direction === "long") {
        if (candles[j].low <= sl) { trades.push({ symbol: sym, dir: "L", entry, exit: sl, r: -1, win: false, bars: j - i }); break }
        if (candles[j].high >= tp) { trades.push({ symbol: sym, dir: "L", entry, exit: tp, r: tpMult, win: true, bars: j - i }); break }
      } else {
        if (candles[j].high >= sl) { trades.push({ symbol: sym, dir: "S", entry, exit: sl, r: -1, win: false, bars: j - i }); break }
        if (candles[j].low <= tp) { trades.push({ symbol: sym, dir: "S", entry, exit: tp, r: tpMult, win: true, bars: j - i }); break }
      }
    }
  }
  return trades
...
async function main() {
  console.log("=== Sniper Mid-Cap Comprehensive Backtest (30 Days) ===\n")
  ...
  // ── Print Results ──
  console.log("\n" + "=".repeat(80))
  console.log("=== AGGREGATE MID-CAP PERFORMANCE (30 DAYS) ===")
  ...
  console.log(`\nProfit Factor:    ${profitFactor} (Gross Wins / Gross Losses)`)
}
