// Liquidity Snipe Backtest: Waits for price to dip below VWAP/EMA, 
// then aggressively reverses. Designed to counter market maker fakeouts.
...
async function run() {
  ...
  console.log(`Testing ${symbol} ${timeframe} Liquidity Trap Strategy...`)
  ...
  for (let i = 50; i < candles.length; i++) {
    ...
    const vw = vwap(candles.slice(Math.max(0, i - 50), i + 1))
    ...
    // Entry Logic: Liquidity Trap
    // Long: Price dips below VWAP and EMA21, but closes back above EMA21. RSI was oversold.
    if (!position) {
      const isUptrend = ema9[i] > ema21[i]
      const dippedBelowVwap = prev.low < vw
      const reclaimedEma = c.close > ema21[i] && prev.close < ema21[i]
      
      if (isUptrend && dippedBelowVwap && reclaimedEma && rsi14[i] > 40 && rsi14[i] < 60) {
        const entry = c.close
        const sl = Math.min(prev.low, c.low) - (atr14[i] * 0.2) // Tight stop below the fakeout
        const tp = entry + (Math.abs(entry - sl) * 2) // 2:1 Risk Reward
