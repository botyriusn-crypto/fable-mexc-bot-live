...
async function fetchKlines(symbol, entryTime) {
  // FIXED: forward window is now 200 candles (16.7h), not 5 minutes
  const end = entryTime + SECONDS * LIMIT
...
console.log("R:R | n | winRate | breakEven | totalR | avgR | stopPct | open")
console.log("----|---|---------|-----------|--------|------|--------|-----")
...
for (const rr of tpSlRatios) {
  ...
  console.log(
    `${rr.toString().padStart(3)} | ${n.toString().padStart(3)} | ${(winRate*100).toFixed(1).padStart(7)}% | ${(breakEven*100).toFixed(1).padStart(9)}% | ${totalR.toFixed(1).padStart(6)} | ${avgR.toFixed(3).padStart(4)} | ${stopPct}% | ${open}${flag}`
  )
}
